            ...
            for i, (image, label, _, name, affine, label_names) in enumerate(infer_loader):
                ...
                if save_infer: # and (round == 0):
                    ...
                    if (curr_epoch == 0):
                        ...
                        scale = 255
                        label_map = self.cli_args.label_index
                        save_img_nifti(image, scale, [], affine, modality,   save_val_path, [f"{mode[:4]}_img"]*len(name))
                        save_seg_nifti(label,        [], affine, label_map,  save_val_path, [f"{mode[:4]}_lbl"]*len(name))
