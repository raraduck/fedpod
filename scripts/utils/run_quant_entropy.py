...
def main(src_base, trg_base):
    ...
    seg_names1 = "[LS,RS]".strip("[]").split(",")
    seg_names2 = "[LVS,LAC,LPC,LAP,LPP,LVP,RVS,RAC,RPC,RAP,RPP,RVP]".strip("[]").split(",")
    case_metrics_meter = CaseSegMetricsMeter(seg_names1, metrics_list=['DICE','HD95','PVDC', 'PRED', 'GOLD'])
    for pid in src_list:
        src_dir = os.path.join(src_path, pid)
        trg_dir = os.path.join(trg_path, pid)
        # print(trg_dir)
        # check_list = os.listdir(trg_dir)
        src_sub_file = os.path.join(src_dir, f"{pid}_seg.nii.gz")
        ...
        trg_sub_vol[trg_sub_vol == 1] = 1
        trg_sub_vol[trg_sub_vol == 2] = 1
        trg_sub_vol[trg_sub_vol == 3] = 1
        trg_sub_vol[trg_sub_vol == 4] = 1
        trg_sub_vol[trg_sub_vol == 5] = 1
        trg_sub_vol[trg_sub_vol == 6] = 1

        trg_sub_vol[trg_sub_vol == 7] = 2
        trg_sub_vol[trg_sub_vol == 8] = 2
        trg_sub_vol[trg_sub_vol == 9] = 2
        trg_sub_vol[trg_sub_vol ==10] = 2
        trg_sub_vol[trg_sub_vol ==11] = 2
        trg_sub_vol[trg_sub_vol ==12] = 2
        ...
        src_sub_vol = F.one_hot(src_sub_vol, num_classes=len(seg_names1)+1)[..., 1:].permute(0, 4, 1, 2, 3).to(torch.float)
        trg_sub_vol = F.one_hot(trg_sub_vol, num_classes=len(seg_names1)+1)[..., 1:].permute(0, 4, 1, 2, 3).to(torch.float)
